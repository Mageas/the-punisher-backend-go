...
func (h *BonusTypeHandler) ListBonusTypes(w http.ResponseWriter, r *http.Request) {
	...
	bonusTypes, totalCount, err := h.service.ListBonusTypes(r.Context(), userID, limit, offset)
	if err != nil {
		web.WriteFromError(w, err)
		return
	}

	response := web.NewPaginatedResponse(bonusTypes, totalCount, page)
...
func (h *BonusTypeHandler) GetBonusType(w http.ResponseWriter, r *http.Request) {
	...
	bonusTypeID, ok := parsePathUUID(w, r, "bonus_type_id", "bonus_type_id", "id")
	if !ok {
		return
	}
...
func (h *BonusTypeHandler) UpdateBonusType(w http.ResponseWriter, r *http.Request) {
	...
	bonusType, err := h.service.UpdateBonusType(r.Context(), userID, bonusTypeID, req)
	if err != nil {
		web.WriteFromError(w, err)
		return
	}

	web.WriteJSON(w, http.StatusOK, bonusType, nil)
}
