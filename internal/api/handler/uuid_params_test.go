...
func TestParseOptionalQueryUUID(t *testing.T) {
	id := uuid.New()
	req := httptest.NewRequest("GET", "/?classroomID="+id.String(), nil)
	rr := httptest.NewRecorder()

	parsed, ok := parseOptionalQueryUUID(rr, req, "classroomID")
	if !ok {
...
func TestParseOptionalQueryUUIDMissing(t *testing.T) {
	req := httptest.NewRequest("GET", "/", nil)
	rr := httptest.NewRecorder()

	parsed, ok := parseOptionalQueryUUID(rr, req, "classroomID")
	if !ok || parsed != nil {
		t.Fatalf("expected nil,true got %v,%v", parsed, ok)
	}
}

func TestParseOptionalQueryUUIDInvalid(t *testing.T) {
	req := httptest.NewRequest("GET", "/?classroomID=invalid", nil)
	rr := httptest.NewRecorder()
	...
	if parsed != nil {
		t.Fatalf("expected nil parsed uuid")
	}
	if rr.Code != 404 {
		t.Fatalf("expected 404, got %d", rr.Code)
	}
